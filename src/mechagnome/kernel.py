...
class ToolSession:
    ...
    def update_metadata(self, **changes: str | None) -> dict[str, Any]:
...
class SessionAccess:
    ...
    def get(self, session_id: str | None = None) -> ToolSession:
        ...
        return ToolSession(self._kernel, self._provider, metadata, self.id)
...
class ToolContext:
    ...
    def __init__(
        ...
        self.sessions = SessionAccess(kernel, provider, state.session_id)
...
class Kernel:
        ...
        with closing(self._connect()) as connection:
            rows = connection.execute(
                ...
                SELECT sessions.id, sessions.cwd, sessions.parent_session_id,
                       ...
                       sessions.description, sessions.created_at,
        ...
        with closing(self._connect()) as connection:
            row = connection.execute(
                ...
                SELECT id, cwd, parent_session_id, kind, origin_session_id,
                       origin_call_id, context_source_session_id,
                       context_through_seq, title, description, created_at
    ...
    def _update_session_metadata(
        ...
        *,
        ...
        """Update mutable display metadata for one session."""
